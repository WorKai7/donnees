...
# ETag pour vérifier si la page a changé pour éviter de surcharger le serveur
try:
    with open("etag.txt", "r", encoding="utf-8") as etag_file:
        etag = etag_file.read().strip()
except:
    etag = None


response = requests.get(base_url + "/newsroom", headers={"If-None-Match": etag})

# Vérification de l'ETag
if response.status_code == 304:
    print("Le fichier XML est déjà à jour")
else:
    if "ETag" in response.headers:
        etag = response.headers["ETag"]

        with open("etag.txt", "w", encoding="utf-8") as etag_file:
